...
export function createNanoGPTChatModel(
  ...
  const textIdGenerator = generateId({ prefix: TEXT_STREAM_ID_PREFIX })
  ...
  return {
    ...
    async doGenerate(options) {
      ...
      const providerMetadata = response.nanoGPT
        ? ({ nanogpt: response.nanoGPT } as SharedV2ProviderMetadata)
        : undefined
      ...
      const processEvent = (rawEvent: string) => {
        ...
        if (trimmed.startsWith('<NanoGPT>') && trimmed.endsWith('</NanoGPT>')) {
          ...
          try {
            ...
            providerMetadata = { nanogpt: parsed }
        ...
        try {
          parsed = JSON.parse(payload) as NanoGPTStreamChunk
        } catch {
          controller.enqueue({ type: 'raw', rawValue: payload })
          return
        }
        ...
        if (parsed.nanoGPT) {
          providerMetadata = { nanogpt: parsed.nanoGPT }
        }
      }
