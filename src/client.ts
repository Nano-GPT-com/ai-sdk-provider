    ...
    if (contentType.includes('application/json')) {
      try {
        const data = (await response.json()) as
          ...
          | { error?: { message?: string; type?: string; code?: string | null; param?: unknown } };

        if ('error' in data && data.error) {
          const nested = data.error;
          return createNanoGPTRequestError({
            code: nested.code ?? nested.type ?? 'unknown_error',
            message: nested.message ?? `Request failed with status ${status}.`,
...
function linkExternalAbort(controller: AbortController, external?: AbortSignal) {
  ...
  if (external.aborted) {
    controller.abort((external as any).reason ?? new DOMException('Aborted', 'AbortError'));
    return () => {};
  }
